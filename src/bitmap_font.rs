/// Public-domain 8×8 ASCII glyphs (space through ~), packed as 8 row bytes.
...
pub fn glyph(ch: char) -> [u8; 8] {
    ...
    let index = mapped as u32;
    if (32..127).contains(&index) {
        GLYPHS[(index - 32) as usize]
    } else {
        GLYPHS[(b'?' - 32) as usize]
    }
}

pub fn render_lines(lines: &[String], scale: u32) -> (u32, u32, Vec<u8>) {
    let scale = scale.max(2);
    let pad = scale;
    let line_h = 8 * scale + scale;
    let max_chars = lines.iter().map(|line| line.chars().count()).max().unwrap_or(1);
    let width = (max_chars as u32 * 8 * scale + pad * 2).max(8);
    let height = (lines.len() as u32 * line_h + pad).max(8);
    let mut rgba = vec![0_u8; width as usize * height as usize * 4];
    for (row, line) in lines.iter().enumerate() {
        let mut x = pad;
        let y = pad + row as u32 * line_h;
        for ch in line.chars() {
            blit_glyph(&mut rgba, width, x, y, scale, glyph(ch));
            x += 8 * scale;
            if x + 8 * scale > width {
                break;
            }
...
fn blit_glyph(rgba: &mut [u8], width: u32, x: u32, y: u32, scale: u32, glyph: [u8; 8]) {
    let outline = (scale / 4).max(1);
    for row in 0..8 {
        for col in 0..8 {
            ...
            fill(
                rgba,
                width,
                px.saturating_sub(outline),
                py.saturating_sub(outline),
                scale + outline * 2,
                scale + outline * 2,
                [0, 0, 0, 255],
            );
            fill(rgba, width, px, py, scale, scale, [255, 255, 255, 255]);
        }
    }
}

fn fill(rgba: &mut [u8], width: u32, x: u32, y: u32, w: u32, h: u32, color: [u8; 4]) {
    for oy in 0..h {
        for ox in 0..w {
            let i = (((y + oy) * width + (x + ox)) as usize) * 4;
            if i + 3 < rgba.len() {
